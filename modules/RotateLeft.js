...
console.log({priorCube})
    ...
    rCube[4].forEach( (e, i) => rCube[4][i] = rCube[4][clockwiseRotator[i]])
}


export const rotateMiddleRowLeft = (rCube) => {
    const priorCube = [];
    rCube.forEach( e => priorCube.push([ ...e]))
...
export const rotateBottomRowLeft = (rCube) => {
    const priorCube = [];
    rCube.forEach( e => priorCube.push([ ...e]))
    //rotateHorizontally facing sides
    //counterClockwiseRotation topRow[2, 5, 8] middleRow[1,4,7] bottomRow[0,3,6]
    ...
    for ( let i = 6; i < 9; i++ ){
        rCube[0].push( [ ...priorCube[3]][i]) 
        rCube[1].push( [ ...priorCube[0]][i])  
        rCube[2].push( [ ...priorCube[1]][i]) 
        rCube[3].push( [ ...priorCube[2]][i])        
        //rotateVertically facing sides
    }
